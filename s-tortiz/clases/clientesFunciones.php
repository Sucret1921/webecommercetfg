...
function login($usuario, $password, $con)
{
    ...
    $sql = "SELECT id, password FROM usuarios WHERE usuario = ? LIMIT 1";
...
{
    $sql = $con->prepare("SELECT activacion FROM usuarios WHERE usuario = ? LIMIT 1");
    if ($sql->execute([$usuario])) {
        $row = $sql->fetch(PDO::FETCH_ASSOC);
        if($row && isset($row['activacion']) && $row['activacion'] == 1) {
            return true;
        }
    }
    return false;
